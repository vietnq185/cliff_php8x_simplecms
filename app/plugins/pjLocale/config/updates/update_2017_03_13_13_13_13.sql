...
SET @id := (SELECT `id` FROM `fields` WHERE `key` = "plugin_locale_ie_body");
UPDATE `multi_lang` SET `content` = 'Use the form below to Import or Export CSV with all titles. Please, do not change first row and first and second column in the CSV file.' WHERE `foreign_id` = @id AND `model` = "pjField" AND `field` = "title";
