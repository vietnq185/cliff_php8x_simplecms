	...
	<form action="<?php echo $_SERVER['PHP_SELF']; ?>?controller=pjAdminSections&amp;action=pjActionUpdate" method="post" id="frmUpdateSection" class="form pj-form" autocomplete="off">
		...
		foreach ($tpl['lp_arr'] as $v)
		{
			...
			<p class="pj-multilang-wrap" data-index="<?php echo $v['id']; ?>" style="display: <?php echo (int) $v['is_default'] === 0 ? 'none' : NULL; ?>">
				...
				<span class="inline_block">
					<input type="text" id="i18n_section_name_<?php echo $v['id'];?>" name="i18n[<?php echo $v['id']; ?>][section_name]" class="pj-form-field w300<?php echo (int) $v['is_default'] === 0 ? NULL : ' required'; ?>" value="<?php echo htmlspecialchars(stripslashes(@$tpl['arr']['i18n'][$v['id']]['section_name'])); ?>" lang="<?php echo $v['id']; ?>" />
			...
			<p class="pj-multilang-wrap" data-index="<?php echo $v['id']; ?>" style="display: <?php echo (int) $v['is_default'] === 0 ? 'none' : NULL; ?>">
				...
				<span class="inline_block">
					<span class="block float_left r5">
						<textarea name="i18n[<?php echo $v['id']; ?>][section_content]" class="mceEditor" lang="<?php echo $v['id']; ?>" ><?php echo htmlspecialchars(stripslashes(@$tpl['arr']['i18n'][$v['id']]['section_content'])); ?></textarea>
					</span>
					<?php if ((int) $tpl['option_arr']['o_multi_lang'] === 1 && count($tpl['lp_arr']) > 1) : ?>
					<span class="pj-multilang-input"><img src="<?php echo $flag_url; ?>" alt="" /></span>
					<?php endif; ?>
