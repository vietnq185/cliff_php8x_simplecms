...
if (isset($tpl['status']))
{
	...
	<form action="<?php echo $_SERVER['PHP_SELF']; ?>?controller=pjAdminUsers&amp;action=pjActionCreate" method="post" id="frmCreateUser" class="form pj-form" autocomplete="off">
		...
		<div class="accessBox">
			<p>
				...
				<span class="inline_block">
					<select name="section_id[]" id="section_id" class="pj-form-field w400" multiple="multiple" size="5">
						...
						{
							?><option value="<?php echo $v['id']; ?>"><?php echo stripslashes($v['section_name']); ?></option><?php
						}
						?>
					</select>
				</span>
			...
			<p>
				...
				<span class="inline_block">
					<select name="file_id[]" id="file_id" class="pj-form-field w400" multiple="multiple" size="5">
						...
						{
							?><option value="<?php echo $v['id']; ?>"><?php echo stripslashes($v['file_name']); ?></option><?php
						}
						?>
					</select>
				</span>
			...
			<p>
				...
				<span class="inline_block">
					<label class="content">
						<input type="checkbox" name="section_allow" id="section_allow" value="1" class="checkbox" />&nbsp;<label for="section_allow"><?php __('lblAllowAddingSections'); ?></label>
					</label>
				</span>
			</p>
			<p>
				...
				<span class="inline_block">
					<label class="content">
						<input type="checkbox" name="file_allow" id="file_allow" value="1" class="checkbox" />&nbsp;<label for="file_allow"><?php __('lblAllowUploadingFiles'); ?></label>
					</label>
				</span>
			</p>
		</div>
	...
	myLabel.select_sections = "<?php __('lblSelectSections');?>";
	myLabel.select_files = "<?php __('lblSelectFiles');?>";
	</script>
	<?php
}
?>