...
if (isset($tpl['status']))
{
	...
	<form action="<?php echo $_SERVER['PHP_SELF']; ?>?controller=pjAdminUsers&amp;action=pjActionUpdate" method="post" id="frmUpdateUser" class="form pj-form">
		...
		<div class="accessBox" style="display:<?php echo $tpl['arr']['role_id'] == '1' ? ' none' : ' block';?>">
			<p>
				<label class="title"><?php __('lblSection'); ?></label>
				<span class="inline_block">
					<select name="section_id[]" id="section_id" class="pj-form-field w400" multiple="multiple" size="5">
						<?php
						foreach ($tpl['section_arr'] as $v)
						{
						    ?><option value="<?php echo $v['id']; ?>"<?php echo in_array($v['id'], $tpl['section_ids_arr']) ? ' selected="selected"' : null;?>><?php echo pjSanitize::html($v['section_name']); ?></option><?php
						}
						?>
					</select>
				</span>
			...
			<p>
				...
				<span class="inline_block">
					<select name="file_id[]" id="file_id" class="pj-form-field w400" multiple="multiple" size="5">
						<?php
						foreach ($tpl['file_arr'] as $v)
						{
						    ?><option value="<?php echo $v['id']; ?>"<?php echo in_array($v['id'], $tpl['file_ids_arr']) ? ' selected="selected"' : null;?>><?php echo pjSanitize::html($v['file_name']); ?></option><?php
						}
						?>
					</select>
				</span>
			...
			<p>
				...
				<span class="inline_block">
					<label class="content">
						<input type="checkbox" name="section_allow" id="section_allow" value="1" class="checkbox"<?php echo $tpl['arr']['section_allow'] == '1' ? ' checked="checked"' : null;?>/>&nbsp;<label for="section_allow"><?php __('lblAllowAddingSections'); ?></label>
					</label>
				</span>
			</p>
			<p>
				...
				<span class="inline_block">
					<label class="content">
						<input type="checkbox" name="file_allow" id="file_allow" value="1" class="checkbox"<?php echo $tpl['arr']['file_allow'] == '1' ? ' checked="checked"' : null;?>/>&nbsp;<label for="file_allow"><?php __('lblAllowUploadingFiles'); ?></label>
					</label>
				</span>
			</p>
		</div>
