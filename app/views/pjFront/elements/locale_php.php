	...
	if(count($tpl['locale_arr']) > 1)
	{
		...
		<div class="scmsLocale">
			...
			foreach ($tpl['locale_arr'] as $locale)
			{
				...
				?><li><a class="scmsSelectorLocale<?php echo $locale_id == $locale['id'] ? ' scmsLocaleFocus' : NULL; ?>" href="<?php echo $_SERVER['SCRIPT_NAME']; ?>?controller=pjFront&amp;action=pjActionSetLocalePhp&amp;locale_id=<?php echo $locale['id']; ?><?php echo isset($_GET['iframe']) ? '&amp;iframe' : NULL; ?>" title="<?php echo pjSanitize::html($locale['title']); ?>"><img src="<?php echo $flag_url; ?>" alt="<?php echo htmlspecialchars($locale['title']); ?>" /></a></li><?php
			}
			?>
			</ul>
		</div>
